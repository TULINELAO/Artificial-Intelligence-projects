...
# ╔═╡ 95803ca0-b11d-11ec-39e8-a972c71f57e7
begin
	using Markdown
	using InteractiveUtils
	using CSV
...
# ╔═╡ 8e1f9263-6505-42f9-8557-b711c581242d
begin
...
# ╔═╡ bc3367cb-c16a-490b-96f0-4b1a45b44440
using EvalMetrics

# ╔═╡ 39444479-5ef0-4744-a138-eac04c6ee497
using Plots

# ╔═╡ 26a03058-b0e3-4b15-9c3e-1d2b3168134a
using MLBase

# ╔═╡ d1bd1f9d-8c64-40a7-9e77-c1be7bc5394c


# ╔═╡ 1e5f678a-8c7e-4b0d-a695-5069bb71d679

raw_df = CSV.read("C:\\Users\\Tulinelao\\Downloads\\real_estate.csv",DataFrame)

# ╔═╡ 377e2d59-db53-44ca-87d1-d697b831ee92
begin
	rename!(raw_df, Symbol.(replace.(string.(names(raw_df)), Ref(r"\[m\]"=>"_"))))
	rename!(raw_df, Symbol.(replace.(string.(names(raw_df)), Ref(r"\[s\]"=>"_"))))
	rename!(raw_df, Symbol.(replace.(string.(names(raw_df)), Ref(r"\s"=>"_"))))
end


# ╔═╡ 7e30df34-36c6-45c7-bba0-97256ec92470
size(raw_df)

# ╔═╡ 6310aca0-f606-443f-a737-13710d0fd901
first(raw_df,20)

# ╔═╡ 8f84f08d-8db3-4406-ba6e-70a266847344
with_terminal() do
	describe(raw_df[!, :X1_transaction_date])
end


# ╔═╡ 97aeb38a-d549-4c96-a1cd-ae9242cc772c
with_terminal() do
	describe(raw_df[!, :X2_house_age])
end


# ╔═╡ bcafffe4-598f-422e-8074-240d7833c353
with_terminal() do
	describe(raw_df[!, :X3_distance_to_the_nearest_MRT_station])
end


# ╔═╡ 1995a5ee-2287-4709-97a8-894a7e6f1f0a
with_terminal() do
	describe(raw_df[!, :X4_number_of_convenience_stores])
end

# ╔═╡ 55ec2da9-9957-41f6-9e07-c81c0c66b50f
with_terminal() do
	describe(raw_df[!, :X5_latitude])
end

# ╔═╡ 5b453b02-91b5-41d8-a3dc-a6fe322fbf95
with_terminal() do
	describe(raw_df[!, :X6_longitude])
end

# ╔═╡ 34d995bb-8f36-41f5-8f8d-a1b259eb6ab8
with_terminal() do
	describe(raw_df[!, :Y_house_price_of_unit_area])
end

# ╔═╡ 12a67df8-40df-499f-b37a-29435645d412
cor(Matrix(raw_df))

# ╔═╡ 30d7885a-e184-41c8-a36d-231ffbe14a59
X = raw_df[:, 3:7]

# ╔═╡ a42f4bb7-286d-4c23-8b70-79df8590e3c2
X_mat = Matrix(X)

# ╔═╡ 24812b4c-359d-4148-a533-61ac9d36bb03
Y = raw_df[:, 8]

# ╔═╡ 76dd5251-d888-4d9d-9215-7c0e8099bcbc
Y_vec = Vector(Y)

# ╔═╡ cabea7a9-ff1e-47b3-95f6-082ba0fca0a4


# ╔═╡ 1a760a1f-9ce5-4667-9c91-eb584e5f7558
training_size = 0.7

# ╔═╡ a39c4ad1-dcf5-456a-bf00-3abf006586c3
all_data_size = size(X_mat)[1]

# ╔═╡ e1489d6f-1ae9-40b0-8153-1e8ca0e1e362
training_index = trunc(Int, training_size * all_data_size)

# ╔═╡ eaa434e9-9090-4012-bb7a-e11db6779c59
X_mat_train = X_mat[1:training_index, :]

# ╔═╡ eb14a060-e5c2-4209-ab58-e40ac0971cf6
X_mat_test = X_mat[training_index+1:end, :]

# ╔═╡ 9c689da2-04a8-4eb7-97bc-75a61abb7943
Y_vec_train = Y_vec[1:training_index]

# ╔═╡ 5d1220d1-dfc9-4170-b956-b842e7af09ac
Y_vec_test = Y_vec[training_index+1:end]

# ╔═╡ cd5617e4-21e0-4f1c-aab1-9a243cd96d41
train_label = Y_vec[1:training_index]

# ╔═╡ a35ce6e8-8304-448e-b0ec-b97fac5faa70
test_label = Y_vec[1+training_index:end]

# ╔═╡ c7a3b561-3158-4d33-8020-0ed2547875b6
hd_m = Chain(Dense(5,3,relu), Dense(4,2,relu),  Dense(2,1,relu))

# ╔═╡ c33c30bc-0b64-4c26-8e33-a9a9e6da71b8
l2_reg(x) = sum(x.^2)

# ╔═╡ 4d10c29a-70fa-4648-940c-1a1709d74912
loss(x, y) = logitcrossentropy(hd_m(x), y) + sum(l2_reg, Flux.params(hd_m))

# ╔═╡ 0bd36191-7244-49ac-9194-b5130f732bf3
ps = Flux.params(hd_m)

# ╔═╡ d3625f1b-d6e6-4791-bec9-32422ed050ab
optimiser = ADAM(0.001)

# ╔═╡ 3a798cf4-a209-4017-a185-f2958f049705
function get_predicted_classes(preds, threshold)
	return preds .>= threshold
end


# ╔═╡ 27b61192-b435-40b0-b3bc-2e9ff61516c2
recall(test_conf_matrix

# ╔═╡ 310565b0-7e67-443b-a7e6-2efb8b21e855


# ╔═╡ a89fcf13-b0b3-4e7b-8e91-2cd936f7b230


# ╔═╡ 3a059d1b-fb18-489d-819a-d7e6833ef855


# ╔═╡ 325659d7-32a4-4783-a1d6-bc18f875f504


# ╔═╡ e46b46c1-2ceb-4589-8a96-87ae0f18d96a
...
# ╔═╡ ed76e8f4-5d6d-4165-82b7-b15f7f1e2e89
...
# ╔═╡ b153615e-3923-4236-a073-0327a3852a69
...
# ╔═╡ 2e4212ea-297a-49bd-9ebc-132419ed27dd
scaling_params = get_scaling_params(X_mat_train)

# ╔═╡ 1fb8213e-5c0a-49d9-b748-910c47a58ac3
scaling_params[1]

# ╔═╡ 2b9c3331-8742-4f3e-888a-130d3ad9de0c
last(scaling_params)

# ╔═╡ 93c14100-d668-4201-a07d-eec3f78dc828
scaled_training_features = scale_features(X_mat_train, scaling_params)

# ╔═╡ 622db2d9-9720-435a-b8ff-89277b1153de
train_feat = transpose(scaled_training_features)

# ╔═╡ ca7e5f96-4f98-47f7-866d-71381a38735e
dl_train = DataLoader((train_feat, train_label), batchsize=50)

# ╔═╡ 40b9620b-dfb0-49e5-b9e7-6bfefbd8c2ef
@epochs 50 Flux.train!(loss, ps, dl_train, optimiser)

# ╔═╡ 8ec21d1b-f69d-45f8-9f13-5bcd886ba588
scaled_testing_features = scale_features(X_mat_test, scaling_params)

# ╔═╡ f411da73-64f6-4a99-9ba2-cdcad8c328d5
test_feat = transpose(scaled_testing_features)

# ╔═╡ 364f07d7-583f-4b1d-8cf6-aee73fd23324
dl_test = DataLoader((test_feat, test_label), batchsize=50)

# ╔═╡ 75446c95-75b8-4529-b9ff-4b7074816111
pred_test = hd_m(test_feat)

# ╔═╡ 6d484804-066c-4d2c-ad19-8a9e4f9a5671
pred_test_classes = get_predicted_classes(pred_test, 0.5)

# ╔═╡ 94ad10db-a30a-4142-a866-8abda8fa65bf
test_conf_matrix = ConfusionMatrix(test_label, onecold(pred_test_classes))

# ╔═╡ ad0f3b7d-2db5-4693-a99b-7ddef15e9e0d
accuracy(test_conf_matrix)

# ╔═╡ 7f9ccfb9-0445-41d8-b03a-086d53c30668
precision(test_conf_matrix)

# ╔═╡ 2c54917f-8b7b-438e-af40-656b7eb814da
...
# ╔═╡ 00634886-57f5-4ffb-a2ab-69567176448f
weights_tr_errors = train_model(scaled_training_features, Y_vec_train, 0.04,8000)

# ╔═╡ 41ad823a-770a-454c-964f-188ad8282f88
...
# ╔═╡ d7112303-1697-45a3-bb89-9f56e1536100
...
# ╔═╡ 35b50619-1fd9-4f36-8593-3ca3b7061b7f
...
# ╔═╡ da757e59-137b-4a98-9f12-731ad6065817
training_predictions = get_predictions(scaled_training_features, weights_tr_errors[1])

# ╔═╡ ca0bf8df-82e7-4304-8c9d-7f3111021993
testing_predictions = get_predictions(scaled_testing_features,weights_tr_errors[1])

# ╔═╡ a0839316-6e37-484b-835c-a98c6ece7400
root_mean_square_error(Y_vec_train, training_predictions)

# ╔═╡ e59203e6-7ce8-4ad6-9d80-ca9d2f142dfe
root_mean_square_error(Y_vec_test, testing_predictions)

# ╔═╡ 05587458-3b08-41ea-8e0f-ad3797c74b3e
