...
# ╔═╡ 9653e480-b387-11ec-0ea6-ed2b56278567
begin
using Markdown
using InteractiveUtils
using Flux
using Flux.Data
using Flux: logitcrossentropy, normalise, onecold, onehotbatch, @epochs
using Statistics
	
end

# ╔═╡ a45ec7c8-376f-4d3d-bf0d-0174e91d77ac
begin
using FileIO
using ImageIO
end

# ╔═╡ 4f4269af-b709-4bd0-972b-3e5e4ac7dcb1
using Images

# ╔═╡ b84183b0-a0a0-4f95-b6e9-f6cad626ea85
begin
using MLDatasets
using Base.Iterators: partition
end

# ╔═╡ 72f5e298-7094-4f09-88df-bfb5a04d2252
using Plots

# ╔═╡ 96db32d2-fc98-4d9e-89cf-40f68ae3da6c
using CUDA

# ╔═╡ 0a46c64d-8cc8-4172-877a-790e5649cb54
using Printf

# ╔═╡ 679ce481-529a-4425-aaa7-de2bb22f4f4b
#md "function to resize the images"

# ╔═╡ d2a52d28-eef3-4d56-83da-49438b58bb24
begin
	base_path = pwd()
	
	file_normal=readdir("C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\train\\NORMAL\\")
	file_pneumonia=readdir("C:\Users\Tulinelao\Desktop\chest_xray\chest_xray\train\PNEUMONIA\\")

	
	path_x_train="C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\train\\NORMAL\\".*file_normal
	path_y_train="C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\train\\PNEUMONIA\\".*file_pneumonia

	x_normal_test = path_x_test
	y_pneumonia_test = path_y_test

	
	file_normal_test=readdir("C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\test\NORMAL\\")
	file_pneumonia_test=readdir(" C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\test\\PNEUMONIA\\")


	
	path_x_normal_test="C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\test\NORMAL\\ ".*file_normal_test
	path_y_pneumonia_test="C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\test\\PNEUMONIA\\".*file_pneumonia_test

	x_train = path_x_normal_train
	y_train  = path_y_pneumonia_train
	x_test_final = [x_normal_train; y_pneumonia_train]
	x_train_final = [path_x_normal_test; path_y_pneumonia_test]
end

# ╔═╡ 110daa50-6271-435a-a3ad-99bd08c26705
##loading labels andimages

# ╔═╡ 482a45bf-b85a-490c-be0c-a09cabc30a73
function get_processed_data(args)
   # Load labels and images
   train_imgs, train_labels = ("C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\train\\NORMAL\\", " C:\Users\Tulinelao\Desktop\chest_xray\chest_xray\train\PNEUMONIA\\")
   mb_idxs = partition(1:length(train_labels), args.batch_size)
   train_set = [make_minibatch(train_imgs, train_labels, i) for i in mb_idxs]
  
   # Prepare test set as one giant minibatch:
   test_imgs, test_labels = ("C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\test\NORMAL\\ "," C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\test\\PNEUMONIA\\")
...
# ╔═╡ deccc2ca-862e-46c8-ac8a-eee9fdbfe4ad


# ╔═╡ 72d60a4b-00f4-4f86-be97-b7abcbf67f87
##Building the model

# ╔═╡ 4ae0ee9f-fc75-4dcd-8971-916f77a4b60a
function build_model(args; imgsize = (28,28,1), nclasses = 10)
   ...
   flatten,
   Dense(prod(cnn_output_size), 9))
end

# ╔═╡ 459f08af-bfcf-4cb9-92e6-79e5460ac273
##trying to get a robust picture
 augment(x) = x .+ gpu(0.1f0*randn(eltype(x), size(x)))

# ╔═╡ 8ef8b403-8102-480c-9e14-4b5f8afad9ae
##computing accurancy of convolutional network

# ╔═╡ ba81d300-1ff8-4e87-9ba9-71242f40a8c0
##Defining train function

# ╔═╡ d65c0b7e-daea-42b1-9d5d-9c79b9eeaba4

function train(; "C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\train\\PNEUMONIA\\ ")   
   args = Args(; " C:\\Users\\Tulinelao\\Desktop\\chest_xray\\chest_xray\\train\\NORMAL\\")
...
# ╔═╡ 541ca75e-7f8f-4f76-982f-fabe4ac2300f
##split data into a training and testing set

# ╔═╡ 6691dabd-0adc-4e40-9169-6d6cccad8766
begin
    labels = vcat([0 for _ in 1:length(x_normal_train)], [1 for _ in 1:length(x_pneumonia_train)])
    (x_train, y_train), (x_test, y_test) = splitobs(shuffleobs((data, labels)), at = 0.8)
end

# ╔═╡ b265fde7-0c33-4abf-a3ef-1d3a7836f83e
##creating a mini batch function for efficient training
##

# ╔═╡ 56d68683-6db5-458c-be75-813236b48105
...
# ╔═╡ 419c2cb4-3361-42c7-a8cf-9a802ab1bca2
##Each minibatch is a tuple of (data, labels).

# ╔═╡ d4ec0b94-7422-43da-ac76-708fc7083e74
begin
...
##all the errors we are getting are because (x_train) pneumonia  & normal are not defined because they failed to load from the directory ask coach for help n

# ╔═╡ 07ca8596-ca00-496e-87c5-2ba274fd6048
##defining the train and test sets

# ╔═╡ c21391ac-8b0b-4e8f-b8dd-d4acc7d53b1e
##convolutional network model with 10 layers ,know function of each layer #10

# ╔═╡ f6f2c24b-b639-49a4-b528-9ae7c7afdf49
model = Chain(
...
# ╔═╡ aa9f8437-289b-4b5c-8cf9-071be66ca98c
##defining functions: loss,accurancy,callback(),optimizer,params

# ╔═╡ ed82a729-b3a4-434e-8b97-85b254da1ea3
##training epochs to check for performance

# ╔═╡ a2f39368-e7df-45f6-912c-5429cf7485ba
@epochs 20 Flux.train!(L, ps, train_set, opt;
               cb = Flux.throttle(update_loss!, 8))

# ╔═╡ 078fd21e-5a43-451b-93f1-f745410895e8
##Plotting graph  for perfomance feedback

# ╔═╡ 086abb12-4747-4bae-a18d-cf82df604110
begin
    plot(train_loss, xlabel="Iterations", title="Image classifier training", label="Train loss", lw=2, alpha=0.9)
    plot!(test_loss, label="Test loss", lw=2, alpha=0.9)
    plot!(acc, label="Accuracy", lw=2, alpha=0.9)
end

# ╔═╡ 81f716b6-2f83-4579-bc8f-ebc42e8c5270
##my next step depends on the kind of accurancy I get from the above graph...if less ##then 80% and theres timme then i have to improvise with epochs ,optimizer and ##learning rate

# ╔═╡ 9903870d-2470-40f4-9c8f-a5a5f1c7747d
opt_2 = ADAM(0.0005)

# ╔═╡ 8e858901-2671-45e0-9233-3bdace2a5aec
## ADAM(0.0005, (0.9, 0.999), IdDict{Any,Any}())

# ╔═╡ 16d699ed-39cc-4bff-9783-1164b2f5ae85
@epochs 5 Flux.train!(L, ps, train_set, opt_2;
               cb = Flux.throttle(update_loss!, 9))

# ╔═╡ f11f00f6-936f-4e6e-8281-70156c15614a


# ╔═╡ 08078835-99ea-48f1-b691-dac1e51327b2
##
## 

# ╔═╡ c0ee6377-8cd9-4c5d-8d1a-e7582d05356e
begin
    train_loss = Float64[]
    test_loss = Float64[]
    acc = Float64[]
...
# ╔═╡ eacf63fa-4b59-47ca-8be1-50220ded2275
